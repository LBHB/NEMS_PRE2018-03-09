...
for j in np.arange(1, 8, 2):
    ...
    for i, cellid in enumerate(cellids):
        ...
        if j == 1:
            if i == 0:
                pred = np.empty(
                    (r.shape[0], r.shape[1], r.shape[2], len(cellids)))
                resp = np.empty(
                    (r.shape[0], r.shape[1], r.shape[2], len(cellids)))
...
for i in range(0, resp.shape[1]):
    for stim in range(0, resp.shape[2]):
        for cell in range(0, resp.shape[-1]):
            cc_rN_all[i, stim, cell] = np.corrcoef(
                rN[:, i, stim, cell], resp[:, i, stim, cell])[0][1]
            cc_r0_all[i, stim, cell] = np.corrcoef(
                pred[:, i, stim, cell], resp[:, i, stim, cell])[0][1]
...
resp_PCA = np.reshape(
    resp_PCA, (bincount_pca * repcount * stimcount, cellcount))
U, S, V = np.linalg.svd(resp_PCA)

plt.subplot2grid((3, 3), (2, 0), colspan=2)
for i in range(0, 5):
    pc = np.mean(np.mean(U[:, i].reshape(
        bincount_pca, repcount, stimcount), 0), 1)
    plt.plot(pc, lw=2, label='pc: %s ,pupil: %s ,model: %s' % (
        (i + 1), round(np.corrcoef(pup_m, pc)[0][1], 2), round(np.corrcoef(diff, pc)[0][1], 2)))
    plt.legend()
...
fs = 100
plt.scatter(np.arange(0, 11), fs *
            np.mean(np.mean(np.mean(resp, 0), 1), 1), c=act_pass)
...
for rep in range(0, repcount):
    ...
    ax.scatter(np.nanmean(r0_perf['bytrial'][rep, :, :], 0), np.nanmean(
        rN_perf['bytrial'][rep, :, :], 0), c=color, s=10, picker=True)
    ax.plot(x, x, '-k', lw=2)
    ax.axis([-.1, 1, -.1, 1])
    ...
    if rep != (ncols * nrows - ncols):
        ...
        ax.set_xlabel('r0')
rawid = 'TAR'
fig.suptitle('r0 vs. rN for each cell \n stim: %s, rawid: %s, spontonly: %s, fs: %s' % (
    stim + 1, rawid, spontonly, fs))
...
for i in range(0, len(keep_inds)):
    ...
    for j in range(0, npcs):
        pcs_sorted[j, it] = np.mean(U[:, j].reshape(
            bincount_pca, repcount, stimcount)[:, i, big_inds])
        pcs_sorted[j, it + 1] = np.mean(U[:, j].reshape(
            bincount_pca, repcount, stimcount)[:, i, big_inds])
...
plt.subplot2grid((3, 3), (2, 0), colspan=2)
for i in range(0, 5):
    plt.plot(pcs_sorted[i, :], lw=2, label='pc: %s ,pupil: %s ,model: %s' % (
        (i + 1), round(np.corrcoef(p_sorted, pcs_sorted[i, :])[0][1], 2), round(np.corrcoef(diff, pcs_sorted[i, :])[0][1], 2)))
    plt.legend()
...
act_pass_unwrapped = list(
    np.tile(a_p, (30, 1)).T.reshape(repcount * stimcount))
...
for i in range(0, npcs):
    pc = np.mean(U[:, i].reshape(bincount_pca, repcount,
                                 stimcount), 0).reshape(repcount * stimcount)
    plt.plot(pc, label='pc: %s ,pupil: %s ,model: %s' % (
        (i + 1), round(np.corrcoef(pupil_unwrapped, pc)[0][1], 2), round(np.corrcoef(diff, pc)[0][1], 2)))
...
plt.xlabel('neurons')
plt.ylabel('neurons')
plt.xticks(np.arange(0, len(c_ids)), np.array(
    (c_ids)), rotation=90, fontsize=8)
...
for i in range(0, nclusters):
    ...
    plt.plot(np.squeeze(np.mean(np.mean(resp_ds[:, :, :, np.argwhere(
        kmeans.labels_ == i)], 2), 2)).T.flatten(), 'k', lw=4)
    plt.axvspan(act[0], act[s_inds[0]] * bincount_ds +
                bincount_ds, color='k', alpha=0.2)
    plt.axvspan(act[s_inds[0] + 1] * bincount_ds, act[-1] *
                bincount_ds + bincount_ds, color='k', alpha=0.2)
    plt.ylabel('rate - Hz')
    plt.xlabel('time')
plt.figure(4)
for i in range(0, nclusters):
    ...
    if len(np.argwhere(kmeans.labels_ == i)) > 1:
        ...
        plt.yticks(range(0, len(np.argwhere(kmeans.labels_ == i))), np.array(
            c_ids)[np.argwhere(kmeans.labels_ == i)], fontsize=6)
    else:
...
cc_rN_clust = cc_rN_all.reshape(
    cc_rN_all.shape[0] * cc_rN_all.shape[1], cc_rN_all.shape[-1])
cc_r0_clust = cc_r0_all.reshape(
    cc_r0_all.shape[0] * cc_r0_all.shape[1], cc_r0_all.shape[-1])
diff_clust = np.empty((nclusters, cc_rN_all.shape[0] * cc_rN_all.shape[1]))
plt.figure()
for i in range(0, nclusters):
    bar_width = 0.35
    diff_clust[i, :] = np.nanmean(np.squeeze(cc_rN_clust[:, np.argwhere(
        kmeans.labels_ == i)] - cc_r0_clust[:, np.argwhere(kmeans.labels_ == i)]), 1)
    plt.bar(i, np.mean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 1)]), bar_width, color='r')
    # /np.sqrt(len((diff_clust[i,np.argwhere(np.repeat(a_p, [stimcount])==1)])))
    sem = np.std(
        (diff_clust[i, np.argwhere(np.repeat(a_p, [stimcount]) == 1)]))
    # /np.sqrt(len((diff_clust[i,np.argwhere(np.repeat(a_p, [stimcount])==0)])))
    sem2 = np.std(
        (diff_clust[i, np.argwhere(np.repeat(a_p, [stimcount]) == 0)]))
    plt.bar(i + bar_width, np.mean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 0)]), bar_width, color='b')
    plt.legend(['active', 'passive'])
    plt.errorbar(i, np.mean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 1)]), yerr=sem, color='k')
    plt.errorbar(i + bar_width, np.mean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 0)]), yerr=sem2, color='k')
...
for i in range(0, cellcount):
    ...
    plt.bar(i, np.nanmean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 1)]), bar_width, color='r')
    sem = np.nanstd((diff_clust[i, np.argwhere(np.repeat(a_p, [stimcount]) == 1)])) / \
        np.sqrt(
            len((diff_clust[i, np.argwhere(np.repeat(a_p, [stimcount]) == 1)])))
    sem2 = np.nanstd((diff_clust[i, np.argwhere(np.repeat(a_p, [stimcount]) == 0)])) / \
        np.sqrt(
            len((diff_clust[i, np.argwhere(np.repeat(a_p, [stimcount]) == 0)])))
    plt.bar(i + bar_width, np.nanmean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 0)]), bar_width, color='b')
    plt.legend(['active', 'passive'])
    plt.errorbar(i, np.nanmean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 1)]), yerr=sem, color='k')
    plt.errorbar(i + bar_width, np.nanmean(diff_clust[i, np.argwhere(
        np.repeat(a_p, [stimcount]) == 0)]), yerr=sem2, color='k')
