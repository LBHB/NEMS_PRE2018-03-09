...
for i, cellid in enumerate(cellids):
    ...
    sys.exit()
    batch = 293
    # "parm50_wcg02_fir10_pupgainctl_fit01_nested5"
    modelname = "parm50_wcg02_fir10_dexp_fit01_nested5"
...
rN = NRF_fit(resp[:, :, np.newaxis, :], r0_strf=pred,
             model='NRF_only', spontonly=0, shuffle=True)
...
for i in range(0, resp.shape[1]):
    for cell in range(0, resp.shape[-1]):
        cc_rN[i, cell] = np.corrcoef(
            fullModel[:, i, cell], resp[:, i, cell])[0][1]
