...
# get rawid
rid = engine.execute('SELECT rawid FROM sCellFile WHERE respfile = "' +
                     respfile + '" AND cellid = %s', (cid,))
for obj in rid:
    rawid = str(obj[0])
isolation = 84
chan_unit_cellid = engine.execute(
    'SELECT channum, unit, cellid FROM gSingleRaw WHERE isolation  > %s AND rawid = %s', (isolation, rawid)).fetchall()
chan_unit_cellid = sorted(chan_unit_cellid, key=lambda x: x[-1])

keep_ind = []
for i in range(0, len(chan_unit_cellid)):
    keep_ind.append(np.argwhere(
        data['cellids'] == np.array(chan_unit_cellid)[:, 2][i]))

keep_ind = [int(s) for s in keep_ind]

trialen = int(data['respFs'] * (data['prestim'] +
                                data['duration'] + data['poststim']))  # remove nans
...
# fit model
n_cvs = 10
rN = NRF.NRF_fit(r, cv_count=n_cvs, model='NRF_PSTH',
                 spontonly=spontonly, shuffle=True)
r0 = NRF.NRF_fit(r, cv_count=n_cvs, model='PSTH_only',
                 spontonly=spontonly, shuffle=True)

# evaluate model performance
# re-sort r and p into by trial (in time) and compute performance across
# trials ind of stim
if bytrial:
...
for stim in range(0, stimcount):
    ...
    for rep in range(0, repcount):
        ...
        ax.scatter(r0_perf['bytrial'][rep, stim, :], rN_perf['bytrial']
                   [rep, stim, :], c=color, s=10, picker=True)
        ax.plot(x, x, '-k', lw=2)
        ax.axis([-.1, 1, -.1, 1])
