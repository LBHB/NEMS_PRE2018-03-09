...
def parm20(stack):
    """
    Loads a 16 channel, 100 Hz BAPHY .mat file with 'parm' marker using the
    provided cellid and batch, and downsamples it to 20 Hz. Does not average over
    response rasters, instead treating each trial as a separate stimulus. 
    Merges all the input data files into a single "file". Then applies
    a 20% estimation/validation split if the est/val datasets are not specified in
    the file.
    ...
    log.info("Initializing load_mat with file {0}".format(file))
    stack.append(nm.loaders.load_mat, est_files=[file], fs=20, 
...
def parm50(stack):
    ...
    file = ut.baphy.get_celldb_file(stack.meta['batch'],stack.meta['cellid'],
                                    fs=100, stimfmt='parm', chancount=16)
    log.info("Initializing load_mat with file {0}".format(file))
    stack.append(nm.loaders.load_mat, est_files=[file], fs=50, 
                 avg_resp=False, merge_files=True)
    stack.append(nm.est_val.crossval, valfrac=0.2)
...
def fchan100(stack):
    ...
    if site=="TAR010c16":
        respfile="{0}{1}_10>74_FA.mat".format(datapath,site)
    else:
        respfile="{0}{1}_10>74.mat".format(datapath,site)
