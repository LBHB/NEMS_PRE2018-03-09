...
def nopupgain(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='nopupgain',
                 fit_fields=['theta'], theta=[0, 1])
...
def pupgainctl(stack):
    ...
    each matrix entry (the same across all entries)
    """
    stack.append(nm.pupil.pupgain, gain_type='linpupgainctl',
                 fit_fields=['theta'], theta=[0, 1, 0, 0])
    mini_fit(stack, mods=['pupil.pupgain'])
...
def pupgain(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='linpupgain',
                 fit_fields=['theta'], theta=[0, 1, 0, 0])
    mini_fit(stack, mods=['pupil.pupgain'])
    print(stack.modules[-1].theta)
...
def stategainctl(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='linpupgainctl',
                 state_var='state', fit_fields=['theta'], theta=[0, 1, 0, 0, 0, 0])
    mini_fit(stack, mods=['pupil.pupgain'])
    print(stack.modules[-1].theta)
...
def behgainctl(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='linpupgainctl',
                 state_var='behavior_condition', fit_fields=['theta'], theta=[0, 1, 0, 0])
    mini_fit(stack, mods=['pupil.pupgain'])
    print(stack.modules[-1].theta)
...
def polypupgain04(stack):  # 4th degree polynomial gain fn
    ...
    are fitted parameters applied to each matrix entry (the same across all entries)
    """
    stack.append(nm.pupil.pupgain, gain_type='polypupgain',
                 fit_fields=['theta'], theta=[0, 0, 0, 0, 0, 1])
    mini_fit(stack, mods=['pupil.pupgain'])
...
def polypupgain03(stack):  # 3rd degree polynomial gain fn
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='polypupgain',
                 fit_fields=['theta'], theta=[0, 0, 0, 0, 1])
    mini_fit(stack, mods=['pupil.pupgain'])
...
def polypupgain02(stack):  # 2nd degree polynomial gain fn
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='polypupgain',
                 fit_fields=['theta'], theta=[0, 0, 0, 1])
    mini_fit(stack, mods=['pupil.pupgain'])
...
def exppupgain(stack):
    ...
    are fitted parameters applied to each matrix entry (the same across all entries)
    """
    stack.append(nm.pupil.pupgain, gain_type='exppupgain',
                 fit_fields=['theta'], theta=[0, 1, 0, 0])
    mini_fit(stack, mods=['pupil.pupgain'])
...
def logpupgain(stack):
    ...
    are fitted parameters applied to each matrix entry (the same across all entries)
    """
    stack.append(nm.pupil.pupgain, gain_type='logpupgain',
                 fit_fields=['theta'], theta=[0, 1, 0, 1])
    mini_fit(stack, mods=['pupil.pupgain'])
...
def powergain02(stack):  # This is equivalent ot what Zach is using
    ...
    are fitted parameters applied to each matrix entry (the same across all entries)
    """
    stack.append(nm.pupil.pupgain, gain_type='powerpupgain',
                 fit_fields=['theta'], theta=[0, 1, 0, 0], order=2)
    mini_fit(stack, mods=['pupil.pupgain'])
...
def butterworth01(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='butterworthHP',
                 fit_fields=['theta'], theta=[1, 25, 0], order=1)
    mini_fit(stack, mods=['pupil.pupgain'])
...
def butterworth02(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='butterworthHP',
                 fit_fields=['theta'], theta=[1, 25, 0], order=2)
    mini_fit(stack, mods=['pupil.pupgain'])
...
def butterworth03(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='butterworthHP',
                 fit_fields=['theta'], theta=[1, 25, 0], order=3)
    mini_fit(stack, mods=['pupil.pupgain'])
...
def butterworth04(stack):
    ...
    """
    stack.append(nm.pupil.pupgain, gain_type='butterworthHP',
                 fit_fields=['theta'], theta=[1, 25, 0], order=4)
    mini_fit(stack, mods=['pupil.pupgain'])
...
def pupwgt(stack, weight_type='linear'):
    ...
    stack.append(nm.filters.weight_channels, output_name="pred2",
                 num_chans=num_chans, phi=phi, parm_type=parm_type)
    stack.modules[-1].phi = phi
    stack.modules[-1].wcoefs = wcoefs
    stack.append(nm.filters.fir, num_coefs=num_coefs,
                 input_name="pred2", output_name="pred2")
    stack.modules[-1].coefs = coefs * 0.99
    stack.modules[-1].baseline = baseline * 0.99

    stack.append(nm.pupil.state_weight, input_name="pred1", input_name2="pred2",
                 weight_type=weight_type, fit_fields=['theta'], theta=[0, 0.01])
    stack.evaluate(wtidx)
