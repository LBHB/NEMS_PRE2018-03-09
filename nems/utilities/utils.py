...
def plot_ssa_idx(m, idx=None, size=FIGSIZE):
    '''
    specific plotting function for the ssa_index module
    '''
    if idx:
        pass
    else:
    ...
    cell_act = {key: (
    np.convolve(np.nanmean(np.concatenate([val for k, val in slice_dict.items() if k[-3:] == key], axis=0), axis=0),
                box, mode='same')) for key in tone_type}

    cell_err = {key: (
    np.convolve(np.nanstd(np.concatenate([val for k, val in slice_dict.items() if k[-3:] == key], axis=0), axis=0),
                box, mode='same')) for key in tone_type}
