...
class weight_channels(nems_module):
    ...
    user_editable_fields = [
            'input_name', 'output_name', 'fit_fields', 'num_dims', 'num_chans',
            'baseline', 'coefs', 'phi', 'parm_fun',
            ]
    plot_fns = [
            nems.utilities.plot.plot_strf,
    ...
    def my_eval(self,X):
        #if not self.d_out:
        #    # only allocate memory once, the first time evaling.
        #    rish is that output_name could change
        if self.parm_fun:
...
class fir(nems_module):
    ...
    (channels,stims,time), convolves with FIR coefficients, applies a baseline
    DC offset, and outputs a 2D stim array (stims,time).
    """
    
    name='filters.fir'
    user_editable_fields = [
            'input_name', 'output_name', 'fit_fields', 'num_dims', 'num_coefs',
            'coefs', 'baseline', 'random_init',
            ]
    plot_fns = [
            nems.utilities.plot.plot_strf,
            nems.utilities.plot.plot_spectrogram,
    ...
    def my_init(self, num_dims=0, num_coefs=20, baseline=0,
                fit_fields=['baseline','coefs'], random_init=False, coefs=None):
        """
    ...
    def my_eval(self,X):
        ...
        for i in range(0, s[0]):
            # TODO: I get what convolve is doing from a general math p.o.v.,
            #       but why is it done here? Probably need to ask SVD for a
    ...
    def get_strf(self):
        ...
        try:
            ...
            p_output_0 = self.parent_stack.modules[wcidx[0]].output_name
            p_output_1 = self.parent_stack.modules[wcidx[1]].output_name
            if len(wcidx) > 0 and p_output_0 == self.output_name:
                wcidx = wcidx[0]
            elif len(wcidx) > 1 and p_output_1 == self.output_name:
                wcidx = wcidx[1]
            else:
                wcidx = -1
        except:
