...
class load_baphy_ssa(nems_module):
    ...
    def evaluate(self, **kwargs):
        ...
        for i in range(d.size):
            ...
            poststim = data['resp'].shape[0] - \
                (prestim + duration) * data['respFs']
            ...
            if m['stimfmt'][0] == 'envelope':
                # stim envelope seems to not be binary
                stim = np.squeeze(m['stim'])
                stim = stim / stim.max()
