...
class crossval(nems_module):
    ...
    user_editable_fields = ['input_name', 'output_name',
                            ...
                            'cv_counter']
    plot_fns = [nems.utilities.plot.raster_plot,
    ...
    def my_init(self, valfrac=-1, interleave_valtrials=True,
                val_mult_repeats=True, keep_filestate=[]):
        # self.field_dict=locals()
        # self.field_dict.pop('self',None)
    ...
    def evaluate(self, nest=0):
        ...
        if valfrac == 0:
            for i, d in enumerate(self.d_in):
                # special case, no validation data, this should just be a
                # pass-through
                d_est = d.copy()
                d_est['est'] = True
                d_est['pred'] = d_est['stim']
...
class crossval_old(nems_module):
    ...
    def my_init(self, valfrac=0, interleave_valtrials=True,
                val_mult_repeats=True):
        # self.field_dict=locals()
        # self.field_dict.pop('self',None)
