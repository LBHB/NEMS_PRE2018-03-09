...
class ADADELTA_min(tf_nems_fitter):
    ...
    def update(self, phi, ticker):
        ...
        # tick=ticker[0]
        # This won't work if the arrays are shaped differently
        stim_up = tf.identity(self.stack.data[-1][ticker]['stim'])
    ...
    def do_fit(self):
        ...
        stim = tf.identity(tf.py_func(
            self.update, [phi, ticker], tf.float32, name='Update'))
