...
class FERReT:
    ...
    def __init__(self,data=None,metadata=None,batch=None,cellid=None,n_coeffs=20,base=0,
                 fit_param={'input_log':['log'],'FIR':['base','coeffs'],'ON':['nonlin'],'pupil_gain':['pupil']},
                            thresh=0.5):
        self.batch=batch
        self.cellid=cellid
...
##FUNCTIONS##
############################################################################### 
    def input_log(self,data):
        #X=copy.deepcopy(self.train['stim'])
        X=data
        v1=self.log[0,0]
        output=np.log(X+v1)
        self.train['stim']=output
        return(output)
   ...
   ############################################################################
    def FIR(self,data): 
        ...
        for i in range(0,s[0]):
            y=np.convolve(X[i,:],self.coeffs[i,:])
            X[i,:]=y[0:X.shape[1]]
        X=X.sum(0)+self.base
        self.current=np.reshape(X,s[1:])
    ...
    def ON(self,**kwargs):  #DEXP output nonlinearity. Working on best way to optimize.
        #ins=copy.deepcopy(self.pred)
        ins=kwargs['data'] #data should be self.pred
        v1=self.nonlin[0,0]
        v2=self.nonlin[0,1]
    ...
    def basic_min(self,function,params,routine='L-BFGS-B',maxit=50000,tiled=True):
        def cost_fn(phi):
            ...
            if function=='FIR': #Modeling fns should be fed ['stim']
                getattr(self,function)(self.train['stim'])
            else: #Other fns should be fed pred, the output from modeling functions
                getattr(self,function)(data=self.pred,pupdata=self.train['pup'])
            mse=self.err(tiled)
            cost_fn.counter+=1
...
#This module compares the effects of a pupil gain module on a "perfect" model created
    ...
    def run_fit(self,queue=('FIR','pupil_gain'),validation=0.05,reps=1):
        self.data_resample(newHz=50,noise_thresh=0.04)
        self.reshape_repetitions()
        self.create_datasets(valsize=validation)
        self.log[0][0]=10
        self.input_log(self.train['stim'])
