...
stim1K=scipy.signal.resample(stim,stim.shape[1]*respFs/stimFs,axis=1)
...
pl.imshow(exampleSG, interpolation='nearest', aspect='auto',origin='lower')
ax=pl.gca()
yt=np.arange(1,15,2)
...
# fh,ax=pl.subplots(4,N)
for ii in range(0,N):
    trialidx=trialset[ii]
    
    ax=pl.subplot(4,N,ii+1)
	# for ax[0,ii].imshow....
    ...
    ax=pl.subplot(4,N,ii+N*3+1)
    psth=np.mean(resp0[:,:,trialidx],axis=1)*respFs
    psth=scipy.signal.resample(psth,psth.shape[0]/5)
